...
def parse_bytecode():
    """upstream a32d9c9f's generate-libjs-bytecode: interpreter.flap -> Op.h + OpCodes.h.

    A THIRD self-built tool, and the one that broke Ulf's build. It replaced
    Meta/Generators/generate_libjs_bytecode_def_derived.py (a Python generator this
    emitter picked up for free, via the Meta/Generators/ pattern) with a second
    binary of the flapc crate -- so the generator moved out of every bucket the
    emitter knew about, and gen_Op simply disappeared from the output. Nothing said
    so: an emitter that enumerates the generator KINDS it understands cannot notice
    a kind going away, only the parity harness can, and only if the harness does not
    silently bucket the command as something else (it did; see COVERED there).

    The ninja command is `<tool> --input F --op-header X.tmp --opcodes-header Y.tmp
    && copy_if_different ... && remove ...`; CMake writes .tmp files and copies them
    so a byte-identical regeneration does not retrigger downstream compiles. Bazel
    genrules are hermetic and declare their outputs, so the .tmp dance is dropped and
    the tool writes $(location ...) directly -- verified byte-identical by the harness.
    """
    for cmd, cd, _declared, _produced in _blocks():
        if not re.search(r'(?:^|&& )\S*bin/generate-libjs-bytecode\b', cmd):
            continue
        # Find the tool in the WHOLE argv, not in the first `&&` segment: CMake
        # emits `cd <dir> && <tool> ... && copy_if_different ...`, so segment 0 is
        # the cd and the tool is in segment 1. Locate by name, then stop at the
        # next `&&` -- position is not something to assume here (the same mistake
        # the glslang parser documents).
        toks = shlex.split(cmd.replace('&&', ' && '))
        i = next(i for i, x in enumerate(toks)
                 if x.endswith('bin/generate-libjs-bytecode'))
        argv = toks[i + 1:]
        if '&&' in argv:
            argv = argv[:argv.index('&&')]
        def val(flag):
            v = argv[argv.index(flag) + 1]
            # CMake passes the outputs relative to the command's cd, and with a
            # .tmp suffix it then copies away; both are CMake's business, not ours.
            v = v[:-4] if v.endswith('.tmp') else v
            return os.path.relpath(os.path.normpath(
                v if v.startswith('/') else os.path.join(cd, v)), FULL)
        return dict(
            input=os.path.relpath(argv[argv.index('--input') + 1], ROOT),
            op_header=val('--op-header'),
            opcodes_header=val('--opcodes-header'),
        )
    return None


def emit_bytecode(bc):
    if not bc:
        return []
    print('    # The LibJS bytecode headers, generated by a tool THIS BUILD MAKES')
    print('    # (//:generate-libjs-bytecode, the flapc crate\'s second bin --')
    print('    # cargo_ring.bzl). Upstream a32d9c9f made interpreter.flap the sole')
    print('    # authority for the instruction set and deleted both Bytecode.def')
    print('    # and the Python generator that read it, which is why the overlay')
    print('    # pinned at f9e34731 failed on a newer checkout: its glob for')
    print('    # Libraries/LibJS/BytecodeDef/** matched nothing.')
    print('    native.genrule(')
    print('        name = %r,' % 'gen_Op')
    print('        srcs = [%r],' % bc['input'])
    print('        outs = [%r, %r],' % (bc['op_header'], bc['opcodes_header']))
    print('        tools = ["//:generate-libjs-bytecode"],')
    print('        cmd = "$(location //:generate-libjs-bytecode) --input $(location %s) '
          '--op-header $(location %s) --opcodes-header $(location %s)",'
          % (bc['input'], bc['op_header'], bc['opcodes_header']))
    print('    )')
    return [bc['op_header'], bc['opcodes_header']]
...
def parse_flap():
    ...
    for cmd, cd, _declared, _produced in _blocks():
        if re.search(r'bin/generate_interpreter_layout', cmd):
            ...
            # --bytecode-def is OPTIONAL, and its absence is the normal case
            # now: upstream a32d9c9f ("LibJS: Derive bytecodes from Flap
            # handlers") made interpreter.flap the sole authority, deleted
            # Bytecode/Bytecode.def and dropped the flag. Read it out of the
            # argv rather than requiring it -- this KeyError'd on the 71fb301a
            # repin, which is the emitter doing its job (a hardcoded flag list
            # is a claim about upstream that upstream can falsify).
            flap = dict(
                ...
                bytecode_def=(os.path.relpath(d['bytecode-def'], ROOT)
                              if 'bytecode-def' in d else None),
...
def emit_flap(layout, flap):
    ...
    srcs = [flap['dsl']]
    if flap['bytecode_def']:
        srcs.append(flap['bytecode_def'])
    srcs.append(':' + layout['out'])
    bcd = (' --bytecode-def $(location %s)' % flap['bytecode_def']
           if flap['bytecode_def'] else '')
    print('    native.genrule(')
    print('        name = %r,' % 'gen_interpreter_asm')
    print('        srcs = [%s],' % ', '.join(repr(s) for s in srcs))
    print('        outs = [%r],' % flap['out'])
    print('        tools = [%r],' % FLAPC_TOOL)
    print('        cmd = "$(location %s) --arch %s --object-format %s '
          '--constants $(location %s)%s '
          '--input $(location %s) --output $@%s",'
          % (FLAPC_TOOL, flap['arch'], flap['object_format'],
             ':' + layout['out'], bcd, flap['dsl'], extra))
    print('    )')
    return [flap['out']]
...
def main():
    ...
    print('# %d glslang shader headers, %d self-built-tool genrules'
          % (len(shaders), (2 if (layout and flap) else 0) + (1 if bytecode else 0)))
    ...
    all_outs.extend(emit_bytecode(bytecode))
