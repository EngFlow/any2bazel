...
# The two cargo workspaces. Flap is `exclude`d from the root workspace and
# resolves its own lock, so it needs the same treatment at a fraction of the
# size. What is IN that lock is deliberately not written down here: this comment
# used to name its three packages, one of which upstream deleted (bytecode_def,
# in a32d9c9f), leaving the emitter to print a confident description of a
# package that no longer existed. emit_ring() reads the lock instead.
LOCKS = ["Cargo.lock", "Libraries/LibJS/Flap/Cargo.lock"]
FLAP_LOCK = "Libraries/LibJS/Flap/Cargo.lock"
...
def emit_ring(crates, specs, binaries):
    ...
    print("    #")
    # The Flap lock's contents are DESCRIBED, not restated. The previous version
    # of this comment said "exactly 3 packages (flapc, in-tree bytecode_def, and
    # smallvec ... =1.15.1)", all four facts hand-copied -- and upstream's
    # a32d9c9f deleted bytecode_def, so the generated file confidently documented
    # a package that no longer exists. Same class as the SYSTEM_LIBS and glib
    # include-root drift: a fact worth stating in generated output is a fact worth
    # reading from the input.
    flap_pkgs = lock_packages(FLAP_LOCK)
    flap_reg, flap_ws = split_packages(flap_pkgs)
    shared = [(n, v) for (n, v, c) in flap_reg
              if (n, v, c) in lock_packages("Cargo.lock")]
    ...
    print("    #                      with %d package%s: %s" % (
        len(flap_pkgs), "" if len(flap_pkgs) == 1 else "s",
        ", ".join("%s %s%s" % (n, v, " (in-tree)" if (n, v) in flap_ws else "")
                  for (n, v) in [(n, v) for (n, v, _) in flap_pkgs])))
    if shared:
        print("    #                      %s %s %s pinned at the same version AND"
              % (", ".join(n for (n, _) in shared),
                 "is" if len(shared) == 1 else "are",
                 "also" if len(shared) == 1 else "also"))
        print("    #                      checksum as the big workspace, so %s the same"
              % ("it is" if len(shared) == 1 else "they are"))
        print("    #                      fetch rule%s, not a second one."
